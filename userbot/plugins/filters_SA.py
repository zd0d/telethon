...
async def on_snip_save(event):
    ...
    if msg:
        ...
        await event.edit(f"- 𖢞 filter {name} saved successfully N . Get it with {name}")
    else:
...
async def on_snip_list(event):
    ...
    OUT_STR = "- 𖢞 Available Filters in the Current Chat :\n"
    if len(all_snips) > 0:
        for a_snip in all_snips:
            OUT_STR += f"👉 {a_snip.keyword} \n"
    else:
        OUT_STR = "- 𖢞 No Filters. Start Saving using `.savefilter`"
    if len(OUT_STR) > 4096:
